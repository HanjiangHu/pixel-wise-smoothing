...
all_classes = sorted(glob.glob("../../metaroom/kinova/test/*"))

motion_type = 'tz' 
if not os.path.exists(f"../../metaroom/kinova/test_kinova_{motion_type}"):
    os.makedirs(f"../../metaroom/kinova/test_kinova_{motion_type}")
for class_path in all_classes:
    for i in range(21):
        if i == 6 or i == 14: continue
        if not os.path.exists(f"../../metaroom/kinova/test_kinova_{motion_type}/{class_path.split('/')[-1]}"):
            os.makedirs(f"../../metaroom/kinova/test_kinova_{motion_type}/{class_path.split('/')[-1]}")
        all_images = sorted(glob.glob(class_path + f"/{i:04d}/video_frame/{motion_type}/*"))

 
        for j in np.random.random_integers(np.floor(len(all_images) * 3 / 8), np.floor(len(all_images) * 5 / 8), 30):
            print(f"../../metaroom/kinova/test_kinova_{motion_type}/{class_path.split('/')[-1]}/" +
                     all_images[j].split('/')[-1][:-4] + f"_{j:03d}" + all_images[j].split('/')[-1][-4:])
            # copyfile(all_images[j], f"../../metaroom/kinova/test_kinova_{motion_type}/{class_path.split('/')[-1]}/" +
            #          all_images[j].split('/')[-1][:-4] + f"_{j:03d}" + all_images[j].split('/')[-1][-4:])


            # Generate 10000 normal random integers with specified mean (loc) and std (scale).
            smoothing = norm.ppf(np.random.random(30), loc=j, scale=np.floor(len(all_images) * 1 / 8)).astype(int)
            smoothing1 = np.where(smoothing < 0, 0, smoothing)
            smoothing2 = np.where(smoothing > len(all_images)-1, len(all_images)-1, smoothing1)
            for k in smoothing2:
                print(f"../../metaroom/kinova/test_kinova_{motion_type}_smoothing/{class_path.split('/')[-1]}/" +
                      all_images[j].split('/')[-1][:-4] + f"_{j:03d}" + f"_{k:03d}" + all_images[j].split('/')[-1][-4:])
