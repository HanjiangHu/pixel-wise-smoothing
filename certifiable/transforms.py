...
class ResolvableProjection:
    ...
    def __init__(self, canopy, sigma, axis, vnn=False):
        self.sigma = sigma
        ...
        if vnn:
            self.intrinsic_matrix[0][0] = self.intrinsic_matrix[0][0] * 2 / 5
            self.intrinsic_matrix[1][1] = self.intrinsic_matrix[1][1] * 4 / 9
            self.intrinsic_matrix[0][2] = self.intrinsic_matrix[0][2] * 2 / 5 - 4
            self.intrinsic_matrix[1][2] = self.intrinsic_matrix[1][2] * 4 / 9 - 4
        self.axis = axis
    ...
    def proc(self, input, empirical=False, type=None):
        ...
        if empirical:
            ...
            alpha = np.random.normal() * self.sigma * 10
