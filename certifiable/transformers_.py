...
class ResolvableProjectionTransformer(AbstractTransformer):
    def __init__(self, sigma, canopy, axis, vnn=False, uniform_smth=False):
        super(ResolvableProjectionTransformer, self).__init__()
        self.projection_adder = transforms.ResolvableProjection(canopy, sigma, axis, vnn=vnn)
        self.sigma = sigma
        self.axis = axis
        self.uniform_smth = uniform_smth
    ...
    def calc_radius(self, pABar: float):
        if self.uniform_smth:
            # uniform smoothing, [-sigma, sigma]
            radius = self.sigma * (2 * pABar - 1)
        else:
            radius = self.sigma * norm.ppf(pABar)
        return radius
...
def gen_transformer(args, canopy, vnn=False, uniform_smth=False) -> AbstractTransformer:
    if args.transtype == 'rotation-noise':
        ...
        # if uniform_smth:
        #     return ResolvableProjectionUniformTransformer(args.noise_sd, canopy, "tz", vnn=vnn, uniform_smth=uniform_smth)
        # else:
        print(f'resolvable_tz')
        return ResolvableProjectionTransformer(args.noise_sd, canopy, "tz", vnn=vnn, uniform_smth=uniform_smth)
    elif args.transtype == 'resolvable_tx':
        print(f'resolvable_tx')
        return ResolvableProjectionTransformer(args.noise_sd, canopy, "tx", vnn=vnn, uniform_smth=uniform_smth)
    elif args.transtype == 'resolvable_ty':
        print(f'resolvable_ty')
        return ResolvableProjectionTransformer(args.noise_sd, canopy, "ty", vnn=vnn, uniform_smth=uniform_smth)
    elif args.transtype == 'resolvable_rz':
        print(f'resolvable_rz')
        return ResolvableProjectionTransformer(args.noise_sd, canopy, "rz", vnn=vnn, uniform_smth=uniform_smth)
    elif args.transtype == 'resolvable_rx':
        print(f'resolvable_rx')
        return ResolvableProjectionTransformer(args.noise_sd, canopy, "rx", vnn=vnn, uniform_smth=uniform_smth)
    elif args.transtype == 'resolvable_ry':
        print(f'resolvable_ry')
        return ResolvableProjectionTransformer(args.noise_sd, canopy, "ry", vnn=vnn, uniform_smth=uniform_smth)
